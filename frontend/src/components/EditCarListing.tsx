...
const EditCarlisting = ({ session, id }: { session: any; id: string }) => {
  ...
  const [formData, setFormData] = useState<UpdateCarProps>({
    _id: "",
    title: "",
    description: "",
    price: 1000,
  ...
  const [errors, setErrors] = useState<Partial<UpdateCarProps>>({});
      ...
      try {
        ...
        currentUser.json().then((data) => {
          setFormData((prevFormData) => ({
            ...prevFormData,
            userRef: data,
            imageUrls: ["https://i.postimg.cc/Ghb8L4bV/car-png-39076.png"],
          }));
        });
        ...
        if (formData.userRef != "") {
          ...
          const response = await fetch(
            `http://localhost:4000/api/listing/update/${carId}`,
            {
              method: "POST",
              headers: {
  ...
  useEffect(() => {
    const getEditCarDetails = async () => {
      const response = await fetch(
        `http://localhost:4000/api/listing/getListing/${id}`
      );
  ...
  return (
    <div className="max-w-lg mx-auto my-10 ">
      <h2 className="text-2xl font-semibold my-8">
        Update Your Listing : {formData.title}
      </h2>
